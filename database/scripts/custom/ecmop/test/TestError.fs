...
(function () {
	...
	var _callFault = function () {
        try {
            _logger.info('Operation test error');
            
            var oc = new Packages.com.hog.noc.ecmop.fs.ErrorRaiser.raise();

        } catch (excp) {
			if (excp instanceof Packages.com.hog.noc.EcmOpException) {
            _logger.error('Test Error found EcmOpExcpetion ' + excp);
			_logger.error('Type ' + excp.getType());
			} else {
			_logger.error('Unknown excp type' + excp);
			}
        }	
    };
