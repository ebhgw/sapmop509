...
(function () {
// Set logging
var _logger = Packages.org.apache.log4j.Logger.getLogger('fs.baselineservizidata');
load('custom/database/DatiecmIf.fs');

// Log startup
_logger.info( "Starting collecting baseline servizi data" )
...
var callback =
{
    ...
    {
		return DatiecmIf.readBaselineServizi() + '';
    }
}
...
session.invokeScript( 'Choose file',
                      clientScript,
                      [ formula.util.makeRemote( callback ), element.name, 'servizi' ],
