...
(function () {
    ...
    var op = 'ASSIGN', message = '', title = '', res, resSum = 0, alarm;

    _logger.info('User ' + currUser + ' started ECM|Take Ownership/Assign Event (multiple)');
    ...
    if (resSum == 0)
        message = 'ECM|Take Ownership took all events'
    else {
        message = 'ECM|Take Ownership failed on some events. See log';
