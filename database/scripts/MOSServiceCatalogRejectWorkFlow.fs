...
try {
	...
	if (this.args &&  this.args!=null &&  this.args!=undefined
		...
		workerscriptname="MOSservicecatalogRejectWorkFlow"
		thatworkerscriptname=formula.logCategory
		formula.logCategory=workerscriptname

		formula.log.debug('starts');
		if (this.alarms && this.alarms[0]) {
			retval=RejectWorkFlow(args, element.dname, this.alarms)
		} else {
			retval=RejectWorkFlow(args, element.dname)
		}
...
// @internal MOSServiceCatalogRejectWorkFlow.fs 1kk5715
