...
//////////////////////////////////////////////////////////////////////////
// @title Send element and alarm information
//
// @define element
// @define alarm
...
load( 'mail/mailalarminfo')

// Do mail element and alarm.
function mailElementAndAlarmInformation( element, sender, server )
{
    // Make the mail message.
	if( this.debug )
		writeln( "mailElementAndAlarmInformation(): constructing new MailElement instance" )
	...
	if( this.debug )
	  writeln( "mailElementAndAlarmInformation(): setting message template" )
    mailElement.setMessageTemplate( 'mailtemplates/element_and_alarm_mail.template' )
    mailElement.send()
}

// Entry point.
if( this.element && this.sender && this.server )
    mailElementAndAlarmInformation( this.element, this.sender, this.server )
else
    throwError( "Unable to send element and alarm information via mail.\nA required script parameter is missing.\n\n" )
